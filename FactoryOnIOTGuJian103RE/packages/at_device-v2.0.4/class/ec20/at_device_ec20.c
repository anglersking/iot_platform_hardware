...
//extern qmtrurc_func ;//指示是否在main中的ec20_urc_qmtrurc_func函数中，如果在，则qmtrurc_func==1
extern int intqmtrurc_func;
//extern int SIMcardDetectionFailed;//3
//extern int DeviceSignalStrengthCheckFailed;//4
//extern int DeviceGSMRegisterFailed;//5
//extern int DeviceGPRSRegisterFailed;//6
extern int errcode;
extern int NetConnected;
extern int OperatorCode;
...
{
#define EC20_LINK_RESP_SIZE     128//最初64
#define EC20_LINK_RESP_TIMO     (3 * RT_TICK_PER_SECOND)
#define EC20_LINK_DELAY_TIME    (120 * RT_TICK_PER_SECOND)//最初30
    ...
    LOG_E("ec20_check_link_status_entry--------------.");
    ...
    while (1)
    {
        ...
        /*如果qmtrurc_func == 1，证明还在处理函数，这个时候就不要进入下面判断是否在线了*/
        if(intqmtrurc_func == 1)
        {
            LOG_E("gqmtrurc_func == 1!Continue!");
...
#define EC20_LINK_THREAD_STACK_SIZE     (1024 + 1024)//最初(1024 + 512)
