...
static int at_recv_readline(at_client_t client)
{
    ...
    while (1)
    {
        at_client_getchar(client, &ch, RT_WAITING_FOREVER);//at_client_getchar(client, &ch, RT_WAITING_FOREVER);
...
int at_client_init(const char *dev_name,  rt_size_t recv_bufsz)
{
    ...
    if (client->device)
    {
        ...
        if (open_result == -RT_EIO)
        {
            ...
            LOG_I("rt_device_open(client->device, RT_DEVICE_OFLAG_RDWR | RT_DEVICE_FLAG_INT_RX success.");
        }
